...
The reference-based homepage, Portfolio index, eight project detail pages, and About page are available in Serbian and English. Services, Contact, and Pricing pages remain outside the implemented scope. Blog is deferred until verified article content is available.
...
Implemented routes: `/sr/`, `/en/`, `/sr/o-nama/`, `/en/about/`, `/sr/portfolio/`, `/en/portfolio/`, and `/sr/portfolio/[slug]/` / `/en/portfolio/[slug]/` for the eight verified projects with case-study content. `/` redirects to `/sr/` with HTTP 307; Next.js normalizes trailing slashes. Unsupported locales, mismatched About locale/slug combinations, and unknown project slugs return 404, including uppercase variants. No browser-language detection or speculative WordPress redirects are used.

Portfolio navigation and the homepage all-projects link point to the localized Portfolio index. Showcase links open local project details, and verified client-site links open externally in the same tab. About links use `/sr/o-nama/` and `/en/about/`, generated by one shared `app/[locale]/[page]/page.tsx` implementation. Only those matching About slugs are published by the translated core-page route; future core pages require changes to its static parameters and the proxy allowlist. Services still points to the localized homepage anchor. Contact and Pricing still open the Serbian WordPress destinations; those temporary links must be replaced before launch.
