...
export function CTA({ locale, copy }: { locale: Locale; copy: Dictionary }) {
  ...
  return (
    <section aria-labelledby="cta-title" className={`${background.grid} ${background.inverseGrid} bg-brand py-12 text-on-brand md:py-16 lg:py-20`}>
      <Container className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center lg:gap-16">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 max-w-xl text-pretty text-on-brand-muted">{content.description}</p>
        </div>
        <Button href={contact.href} external={contact.external} variant="secondary" className="min-h-14 justify-self-start px-6 focus-visible:outline-accent">
