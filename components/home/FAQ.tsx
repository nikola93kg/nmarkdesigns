...
export function FAQ({ locale, copy }: { locale: Locale; copy: Dictionary }) {
  ...
  return (
    <section aria-labelledby="faq-title" className="py-section lg:py-24">
      <Container className="grid gap-8 text-center lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:gap-16 lg:text-left">
        <div>
          <p className="mb-4 text-small font-medium text-brand">{content.eyebrow}</p>
          <h2 id="faq-title" className="mx-auto max-w-md text-heading font-bold text-ink lg:mx-0">{content.title}</h2>
          <p className="mx-auto mt-4 max-w-sm text-pretty text-muted lg:mx-0">{content.description}</p>
