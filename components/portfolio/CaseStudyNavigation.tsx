...
export function CaseStudyNavigation({ nextProject, locale, copy }: { nextProject?: Project; locale: Locale; copy: Dictionary }) {
  return (
    <nav aria-label={copy.caseStudy.navigation} className="border-t border-border bg-surface-muted py-8 md:py-10">
      <Container className="flex flex-col items-center justify-between gap-6 text-center lg:flex-row lg:items-center lg:gap-12 lg:text-left">
        ...
        {nextProject && (
          <Link href={localizedProjectPath(nextProject.slug, locale)} className="group flex min-h-11 min-w-0 flex-col items-center gap-3 text-brand lg:flex-row lg:gap-6 lg:text-right">
